...
bool Camera::init( std::string calibFilename )
{
    ...
    fs["camera_matrix"] >> Camera::matK;

    // load the distortion_coefficients in distCoeff
    fs["distortion_coefficients"] >> Camera::distCoeff;

    // load image_width and image_height in imageSize.[width|height]
    fs["image_width"] >> Camera::imageSize.width;
    fs["image_height"] >> Camera::imageSize.height;

    cout << Camera::matK << endl;
    cout << Camera::distCoeff << endl;
    cout << Camera::imageSize << endl;
